...
export async function handleAddBookmark(userId, videoId) {
  try {
    ...
    if (isAlreadyBookmarked) {
      Alert.alert("You have already bookmarked this video.");
      return;
    }

    // Add the user's ID to the bookmarks array
    const updatedBookmarks = [...video.bookmarks, userId];

    // Update the video document with the new bookmarks array
    ...
    Alert.alert("An error occurred while adding the bookmark.");
  }
}
